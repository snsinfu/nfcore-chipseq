...
- `<ALIGNER>/merged_library/macs3/<PEAK_TYPE>/`
  - `*.mLb.clN_peaks.xls`, `*.mLb.clN_peaks.broadPeak` or `*.mLb.clN_peaks.narrowPeak`, `*.mLb.clN_peaks.gappedPeak`, `*summits.bed`: MACS3 output files - the files generated will depend on whether MACS3 has been run in _narrowPeak_ or _broadPeak_ mode.
  - `*.mLb.clN_peaks.annotatePeaks.txt`: HOMER peak-to-gene annotation file.
- `<ALIGNER>/merged_library/macs3/<PEAK_TYPE>/qc/`
  - `macs3_peak.mLb.clN.plots.pdf`: QC plots for MACS3 peaks.
  - `macs3_annotatePeaks.mLb.clN.plots.pdf`: QC plots for peak-to-gene feature annotation.
  - `*.mLb.clN.FRiP_mqc.tsv`, `*.mLb.clN.peak_count_mqc.tsv`, `macs3_annotatePeaks.mLb.clN.summary_mqc.tsv`: MultiQC custom-content files for FRiP score, peak count and peak-to-gene ratios.
- `<ALIGNER>/merged_library/downsample/`
  - `*.downsample_summary.txt`: Per-sample downsampling summary, only produced when `--downsample_fragments` is set. Columns: `sample`, `total_fragments`, `target_fragments`, `fraction`, `retained_fragments`, `downsampled`, `control_fragments`, `depth_limited`.
...
[MACS3](https://github.com/macs3-project/MACS) is one of the most popular peak-calling algorithms for ChIP-seq data. By default, the peaks are called with the MACS3 `--broad` parameter. If, however, you would like to call narrow peaks then please provide the `--narrow_peak` parameter when running the pipeline. When `--downsample_fragments` is set, the peaks (and the FRiP score) are computed from alignments downsampled to a fixed fragment depth, while the consensus peaks and the featureCounts/DESeq2 analyses below are still based on the full-depth alignments. See [MACS3 outputs](https://github.com/macs3-project/MACS/blob/master/docs/callpeak.md#output-files) for a description of the output files generated by MACS3.
...
- `<ALIGNER>/merged_library/macs3/<PEAK_TYPE>/consensus/<ANTIBODY>/`
  - `*.mLb.clN.bed`: Consensus peak-set across all samples in BED format.
  - `*.mLb.clN.saf`: Consensus peak-set across all samples in SAF format. Required by featureCounts for read quantification.
  - `*.mLb.clN.featureCounts.txt`: Read counts across all samples relative to consensus peak-set.
  - `*.mLb.clN.annotatePeaks.txt`: HOMER peak-to-gene annotation file for consensus peaks.
  - `*.mLb.clN.boolean.annotatePeaks.txt`: Spreadsheet representation of consensus peak-set across samples **with** gene annotation columns. The columns from individual peak files are included in this file along with the ability to filter peaks based on their presence or absence in multiple replicates/conditions.
  - `*.mLb.clN.boolean.txt`: Spreadsheet representation of consensus peak-set across samples **without** gene annotation columns. Same as file above but without annotation columns.
  - `*.mLb.clN.boolean.intersect.plot.pdf`, `*.mLb.clN.boolean.intersect.txt`: [UpSetR](https://cran.r-project.org/web/packages/UpSetR/README.html) files to illustrate peak intersection.
...
- `<ALIGNER>/merged_library/macs3/<PEAK_TYPE>/consensus/<ANTIBODY>/deseq2/`
  - `*.mLb.clN.sample.dists.txt`: Spreadsheet containing sample-to-sample distance across each consensus peak.
  - `*.mLb.clN.plots.pdf`: File containing PCA and hierarchical clustering plots.
  - `*.mLb.clN.dds.RData`: File containing R `DESeqDataSet` object generated by DESeq2, with either
    an rlog or vst `assay` storing the variance-stabilised data.
  - `*.mLb.clN.rds`: Alternative version of the RData file suitable for
    `readRDS` to give user control of the eventual object name.
  - `*.mLb.clN.pca.vals.txt`: Matrix of values for the first 2 principal components.
...
- `<ALIGNER>/merged_replicate/downsample/`
  - `*.downsample_summary.txt`: Per-group downsampling summary of the pooled merged-replicate alignment, only produced when `--downsample_fragments` is set. Same columns as the merged-library summary.
